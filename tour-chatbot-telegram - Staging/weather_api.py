...
def weather_today(city_gubun):
    ...
    while True:
        try:
            ...
            base_datetime = datetime.strptime(base_date+base_time, "%Y%m%d%H%M").strftime("%Y년 %m월 %d일 %H시 %M분".encode('unicode-escape').decode()).encode().decode('unicode-escape')
            forecast_datetime = datetime.strptime(forecast_date+forecast_time, "%Y%m%d%H%M").strftime("%Y년 %m월 %d일 %H시 %M분".encode('unicode-escape').decode()).encode().decode('unicode-escape')
